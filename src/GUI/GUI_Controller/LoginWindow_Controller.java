package GUI.GUI_Controller;

import Database.EmployeeLoginVerifier;
import javafx.fxml.FXML;
import javafx.fxml.FXMLLoader;
import javafx.fxml.Initializable;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.control.TextField;
import javafx.stage.Stage;

import java.io.IOException;
import java.net.URL;
import java.util.ResourceBundle;

/**
 * Created by roije on 09/05/2016.
 */
public class LoginWindow_Controller implements Initializable
{

    @FXML
    private TextField empNumField;

    @FXML
    private Label errorLabel;

    @FXML
    private Button submitBtn;

    @Override
    public void initialize(URL location, ResourceBundle resources)
    {
        errorLabel.setVisible(false);
    }

    public void resetLabel()
    {
        errorLabel.setVisible(false);
    }

    public void checkLogin()
    {
        EmployeeLoginVerifier verifier = new EmployeeLoginVerifier();

        if (verifier.verifyUser(empNumField))
        {

            FrontPage_Controller frontPage_controller = new FrontPage_Controller();
            Stage frontPageStage = (Stage) frontPage_controller.getLoginBtn().getScene().getWindow();
            Stage loginStage = (Stage) submitBtn.getScene().getWindow();
            loginStage.close();
            Parent root = null;
            try
            {
                root = FXMLLoader.load(getClass().getResource("/GUI/FXML_HomePage.fxml"));
            } catch (IOException e)
            {
                e.printStackTrace();
            }
            Scene scene = new Scene(root);
            frontPageStage.setScene(scene);
            frontPageStage.show();

        }
        else
        {
            errorLabel.setVisible(true);
        }
    }

}
